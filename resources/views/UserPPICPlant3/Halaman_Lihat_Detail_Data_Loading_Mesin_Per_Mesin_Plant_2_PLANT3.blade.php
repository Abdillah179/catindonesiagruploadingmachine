        ...
        <div class="row">
            ...
            <div id="sidebar" class="col-md-2 sidebar d-none d-md-block">
                ...
                <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="/pilihopsilihatdataloadingmachineplant2/plant3">Lihat Data Loading Machine Plant 2</a>
            ...
            <div id="mainContent" class="col-md-10 main-content">
                ...
                <h2 class="mb-4 judul-h2">Detail Data Loading Mesin {{ $mesin->nama_mesin }} Plant 2</h2>
                     ...
                     <div class="col-12 mb-5">
                        <div class="card">
                            <div class="card-header">
                                <form action="/lihatdetaildataloadingmesinpermesinplant2/plant3/{{ $mesin->url_unique_mesin }}">
                                    ...
                                    <div class="row align-items-end mt-3">
                                        ...
                                        <div class="col-md-2 mb-3">
                                            ...
                                            <a href="/lihatdetaildataloadingmesinpermesinplant2/plant3/{{ $mesin->url_unique_mesin }}" class="btn btn-warning mt-4">Clear</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
                   ...
                   <div class="col-12 col-md-8 col-lg-12 mb-4 mx-auto" style="max-width: 2000px">
                    <div class="container">
                        ...
                        <div class="info">
                            ...
                            <div class="info-item">Jumlah Data : {{ $dataloadingmesinplant2count }}</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach($dataloadingmesinplant2 as $index => $dtloadingmesinplant2)
                                    <tr>
                                        <td>{{ $dataloadingmesinplant2->firstItem() + $index }}</td>
                                        
                                        <td>
                                            @php
                                                $datafileprojek = DB::table('tb_data_file_projek_loading_machine_plant_2s')->where('url_unique_loading_machine', $dtloadingmesinplant2->url_unique)->exists();
                                            @endphp

                                            @if($datafileprojek)

                                                <!-- Button trigger modal -->
                                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModalLihatDataFileProjekLoadingMachinePerMachinePlant2PLNT2{{ $dtloadingmesinplant2->url_unique }}">
                                                    Lihat File Projek
                                                </button>
                                                
                                                <!-- Modal -->
                                                <div class="modal fade" id="exampleModalLihatDataFileProjekLoadingMachinePerMachinePlant2PLNT2{{ $dtloadingmesinplant2->url_unique }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-xl modal-dialog-scrollable">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                @foreach($dtloadingmesinplant2->fileProjekLoadingMachinePlant2 as $dtfileprojek)
                                        ...
                                        <td>{{ $dtloadingmesinplant2->plant }}</td>
                                        <td>{{ $dtloadingmesinplant2->nama_mesin }}</td>

                                        @if($dtloadingmesinplant2->status_done == 'Not Done')
                                        <td>
                                            <span class="badge text-bg-danger">Not Done</span>
                                        </td>
                                        @elseif($dtloadingmesinplant2->status_done == 'Done')
                                        <td>
                                            <span class="badge text-bg-primary">Done</span>
                                        </td>
                                        @endif

                                        <td>{{ $dtloadingmesinplant2->project }}</td>
                                        <td>{{ $dtloadingmesinplant2->customer }}</td>
                                        <td>{{ $dtloadingmesinplant2->no_spk }}</td>
                                        <td>{{ $dtloadingmesinplant2->qty }}</td>
                                        <td>{{ $dtloadingmesinplant2->estimasi_jam }}</td>
                                        <td>{{ $dtloadingmesinplant2->actual_jam }}</td>
                                        <td>{{ $dtloadingmesinplant2->start }}</td>
                                        <td>{{ $dtloadingmesinplant2->target_finish }}</td>
                                        <td>{{ $dtloadingmesinplant2->priority }}</td>
                                        <td>{{ $dtloadingmesinplant2->on_proses }}</td>
                                        <td>{{ $dtloadingmesinplant2->operator }}</td>
                                        <td>{{ $dtloadingmesinplant2->keterangan }}</td>
                                        <td>{{ $dtloadingmesinplant2->tanggal_input }}</td>

                                        @if($dtloadingmesinplant2->user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant2->departemen_user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant2->plant_user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant2->tanggal_updated_status_done_loading_mesin && $dtloadingmesinplant2->jam_updated_status_done_loading_mesin)
                                        
                                        <td>
                                            <span class="badge text-bg-primary mb-2 span-badge">Status Done Loading Mesin Di Update Oleh : {{ $dtloadingmesinplant2->user_pengupdated_status_done_loading_mesin }}</span>
                                            <span class="badge text-bg-primary mb-2 span-badge">Plant User Pengupdated : {{ $dtloadingmesinplant2->plant_user_pengupdated_status_done_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Tanggal Updated : {{ $dtloadingmesinplant2->tanggal_updated_status_done_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Jam Updated : {{ $dtloadingmesinplant2->jam_updated_status_done_loading_mesin }}</span>
                                        </td>
                                        @elseif($dtloadingmesinplant2->user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant2->departemen_user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant2->plant_user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant2->tanggal_updated_status_done_loading_mesin == null && $dtloadingmesinplant2->jam_updated_status_done_loading_mesin == null)
                                        <td></td>
                                        @endif


                                        @if($dtloadingmesinplant2->user_pengupdated_data_loading_mesin && $dtloadingmesinplant2->departemen_user_pengupdated_data_loading_mesin && $dtloadingmesinplant2->plant_user_pengupdated_data_loading_mesin && $dtloadingmesinplant2->tanggal_updated_data_loading_mesin && $dtloadingmesinplant2->jam_updated_data_loading_mesin)

                                        <td>
                                            <span class="badge text-bg-primary mb-2 span-badge">Data Loading Mesin Di Edit Oleh : {{ $dtloadingmesinplant2->user_pengupdated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Plant User Pengedit : {{ $dtloadingmesinplant2->plant_user_pengupdated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Tanggal Diedit : {{ $dtloadingmesinplant2->tanggal_updated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Jam Diedit : {{ $dtloadingmesinplant2->jam_updated_data_loading_mesin }}</span> 
                                        </td>
                                        @elseif(!$dtloadingmesinplant2->user_pengupdated_data_loading_mesin && !$dtloadingmesinplant2->departemen_user_pengupdated_data_loading_mesin && !$dtloadingmesinplant2->plant_user_pengupdated_data_loading_mesin && !$dtloadingmesinplant2->tanggal_updated_data_loading_mesin && !$dtloadingmesinplant2->jam_updated_data_loading_mesin)
                                        <td></td>
                                        @endif
                                    </tr>
                                    @endforeach
...
<script>
    ...
    function exportPDF() {
        ...
        let url = "/exportpdfdatadetailloadingmesinplant2permesin/plant3/{{ $mesin->url_unique_mesin }}";
...
<script>
    function exportExcel() {
        ...
        let endDate = document.querySelector('input[name="end_date"]').value; // Ganti dengan selector yang sesuai
        let statusFilter = document.querySelector('select[name="opsi_filter"]').value; // Ganti dengan selector yang sesuai
        let url = "/exportexceldatadetailloadingmesinplant2permesin/plant3/{{ $mesin->url_unique_mesin }}";
