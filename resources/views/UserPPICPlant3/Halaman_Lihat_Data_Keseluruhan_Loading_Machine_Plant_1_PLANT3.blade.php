        ...
        <div class="row">
            ...
            <div id="sidebar" class="col-md-2 sidebar d-none d-md-block">
                ...
                <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="/pilihopsilihatdataloadingmachineplant1/plant3">Lihat Data Loading Machine Plant 1</a>
            ...
            <div id="mainContent" class="col-md-10 main-content">
                ...
                <h2 class="mb-4 judul-h2">Lihat Data Keseluruhan Loading Machine Plant 1</h2>
                ...
                <div class="row row-card">
                    <div class="col-12 mb-5">
                        <div class="card">
                            <div class="card-header">
                                <form action="/lihatdatakeseluruhanloadingmachineplant1/plant3">
                                    ...
                                    <div class="row align-items-end mt-3">
                                        ...
                                        <div class="col-md-2 mb-3">
                                            ...
                                            <a href="/lihatdatakeseluruhanloadingmachineplant1/plant3" class="btn btn-warning mt-4">Clear</a>
                                        </div>
                                    </div>
                                </form>
                            </div>
                   ...
                   <div class="col-12 col-md-8 col-lg-12 mb-4 mx-auto" style="max-width: 2000px">
                    <div class="container">
                        ...
                        <div class="info">
                            <div class="info-item">Jumlah Data : {{ $dataloadingmesinplant1count }}</div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    @foreach($dataloadingmesinplant1 as $index => $dtloadingmesinplant1)
                                    <tr>
                                        <td>{{ $dataloadingmesinplant1->firstItem() + $index }}</td>
                                        
                                        <td>
                                            @php 
                                                $datafileprojek = DB::table('tb_data_file_projek_loading_machine_plant_1s')->where('url_unique_loading_machine', $dtloadingmesinplant1->url_unique)->exists();
                                            @endphp

                                            @if($datafileprojek)
                                                <!-- Button trigger modal -->
                                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModalLihatDataFileProjekLoadingMachineKeseluruhanPlant1PLANT3{{ $dtloadingmesinplant1->url_unique }}">
                                                    Lihat File Projek
                                                </button>
                                                
                                                <!-- Modal -->
                                                <div class="modal fade" id="exampleModalLihatDataFileProjekLoadingMachineKeseluruhanPlant1PLANT3{{ $dtloadingmesinplant1->url_unique }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                    <div class="modal-dialog modal-xl modal-dialog-scrollable">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                @foreach($dtloadingmesinplant1->fileProjek as $dtfileprojek)
                                        ...
                                        <td>{{ $dtloadingmesinplant1->plant }}</td>
                                        <td>{{ $dtloadingmesinplant1->nama_mesin }}</td>

                                        @if($dtloadingmesinplant1->status_done == 'Not Done')
                                        <td>
                                            <span class="badge text-bg-danger">Not Done</span>
                                        </td>
                                        @elseif($dtloadingmesinplant1->status_done == 'Done')
                                        <td>
                                            <span class="badge text-bg-primary">Done</span>
                                        </td>
                                        @endif

                                        <td>{{ $dtloadingmesinplant1->project }}</td>
                                        <td>{{ $dtloadingmesinplant1->customer }}</td>
                                        <td>{{ $dtloadingmesinplant1->no_spk }}</td>
                                        <td>{{ $dtloadingmesinplant1->qty }}</td>
                                        <td>{{ $dtloadingmesinplant1->estimasi_jam }}</td>
                                        <td>{{ $dtloadingmesinplant1->actual_jam }}</td>
                                        <td>{{ $dtloadingmesinplant1->start }}</td>
                                        <td>{{ $dtloadingmesinplant1->target_finish }}</td>
                                        <td>{{ $dtloadingmesinplant1->priority }}</td>
                                        <td>{{ $dtloadingmesinplant1->on_proses }}</td>
                                        <td>{{ $dtloadingmesinplant1->operator }}</td>
                                        <td>{{ $dtloadingmesinplant1->keterangan }}</td>
                                        <td>{{ $dtloadingmesinplant1->tanggal_input }}</td>

                                        @if($dtloadingmesinplant1->user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant1->departemen_user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant1->plant_user_pengupdated_status_done_loading_mesin && $dtloadingmesinplant1->tanggal_updated_status_done_loading_mesin && $dtloadingmesinplant1->jam_updated_status_done_loading_mesin)
                                        
                                        <td>
                                            <span class="badge text-bg-primary mb-2 span-badge">Status Done Loading Mesin Di Update Oleh : {{ $dtloadingmesinplant1->user_pengupdated_status_done_loading_mesin }}</span>
                                            <span class="badge text-bg-primary mb-2 span-badge">Plant User Pengupdated : {{ $dtloadingmesinplant1->plant_user_pengupdated_status_done_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Tanggal Updated : {{ $dtloadingmesinplant1->tanggal_updated_status_done_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Jam Updated : {{ $dtloadingmesinplant1->jam_updated_status_done_loading_mesin }}</span> 
                                        </td>
                                        @elseif($dtloadingmesinplant1->user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant1->departemen_user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant1->plant_user_pengupdated_status_done_loading_mesin == null && $dtloadingmesinplant1->tanggal_updated_status_done_loading_mesin == null && $dtloadingmesinplant1->jam_updated_status_done_loading_mesin == null)
                                        <td></td>
                                        @endif


                                        @if($dtloadingmesinplant1->user_pengupdated_data_loading_mesin && $dtloadingmesinplant1->departemen_user_pengupdated_data_loading_mesin && $dtloadingmesinplant1->plant_user_pengupdated_data_loading_mesin && $dtloadingmesinplant1->tanggal_updated_data_loading_mesin && $dtloadingmesinplant1->jam_updated_data_loading_mesin)

                                        <td>
                                            <span class="badge text-bg-primary mb-2 span-badge">Data Loading Mesin Di Edit Oleh : {{ $dtloadingmesinplant1->user_pengupdated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Plant User Pengedit : {{ $dtloadingmesinplant1->plant_user_pengupdated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Tanggal Diedit : {{ $dtloadingmesinplant1->tanggal_updated_data_loading_mesin }}</span> 
                                            <span class="badge text-bg-primary mb-2 span-badge">Jam Diedit : {{ $dtloadingmesinplant1->jam_updated_data_loading_mesin }}</span> 
                                        </td>
                                        @elseif(!$dtloadingmesinplant1->user_pengupdated_data_loading_mesin && !$dtloadingmesinplant1->departemen_user_pengupdated_data_loading_mesin && !$dtloadingmesinplant1->plant_user_pengupdated_data_loading_mesin && !$dtloadingmesinplant1->tanggal_updated_data_loading_mesin && !$dtloadingmesinplant1->jam_updated_data_loading_mesin)
                                        <td></td>
                                        @endif
                                    </tr>
                                    @endforeach
...
<script>
    ...
    function exportPDF() {
        ...
        let url = "/exportpdfdatakeseluruhanloadingmachineplant1/plant3";
...
<script>
    function exportExcel() {
        ...
        let endDate = document.querySelector('input[name="end_date"]').value; // Ganti dengan selector yang sesuai
        let statusFilter = document.querySelector('select[name="opsi_filter"]').value; // Ganti dengan selector yang sesuai
        let url = "/exportexceldatakeseluruhanloadingmachineplant1/plant3";
