...
<body>
        ...
        <div class="row">
            ...
            <div id="sidebar" class="col-md-2 sidebar d-none d-md-block">
                ...
                <nav class="nav flex-column">
                    ...
                    <a class="nav-link active" href="/datauserppicplant3">Data User PPIC Plant 3</a>
            ...
            <div id="mainContent" class="col-md-10 main-content">
                ...
                <h2 class="mb-4">Data User PPIC Plant 3</h2>
                ...
                <div class="row row-card">
                    ...
                    @if(session('PostDeleteDataUserPPICPlant3ADM'))
                    <div class="alert alert-primary alert-dismissible fade show" role="alert">
                        <strong>{{ session('PostDeleteDataUserPPICPlant3ADM') }}</strong>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                    @endif

                    @if(session('PostTambahDataUserPPICPlant3ADM'))
                    <div class="alert alert-primary alert-dismissible fade show" role="alert">
                        <strong>{{ session('PostTambahDataUserPPICPlant3ADM') }}</strong>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                    @endif

                    <div class="col-12 col-md-8 col-lg-12 mb-4">
                        <form action="/datauserppicplant3" method="GET">
                    ...
                    <div class="col-12 col-md-8 col-lg-12 mb-4">
                        ...
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">
                            <i class="fa-solid fa-plus"></i> Tambah Data User PPIC Plant 3
                        </button>
                        
                        <h5 class="mt-4">Jumlah Data : {{ $datauserppicplant3count }}</h5>
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                            <div class="modal-content">
                                <div class="modal-header">
                                <h1 class="modal-title fs-5" id="exampleModalLabel">Tambah Data User PPIC Plant 3</h1>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <form action="/posttambahdatauserppicplant3/adm" method="post" enctype="multipart/form-data">
                                        @csrf
                                        @method('POST')
                                        ...
                                        <div class="mb-3">
                                          ...
                                          <input type="text" class="form-control" id="exampleInputPlant" name="plant" value="Plant 3" readonly>
                                        </div>
                                        <div class="mb-3">
                                        ...
                                        <div class="mb-3">
                                            <label for="imageppicplant3" class="form-label">Pas Foto</label>

                                            <img class="img-preview img-fluid mb-2">
                                            <input type="file" class="form-control" id="imageppicplant3" name="image" onchange="previewImage()">
                                        </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php $no = 1;?>
                                @foreach($datauserppicplant3 as $index => $dtuserppicplant3)
                              <tr>
                                <td>{{ $datauserppicplant3->firstItem() + $index }}</td>
                                <td>{{ $dtuserppicplant3->nama_user }}</td>
                                <td>{{ $dtuserppicplant3->nik_karyawan }}</td>
                                <td>{{ $dtuserppicplant3->no_telepon }}</td>
                                <td>{{ $dtuserppicplant3->departemen }}</td>
                                <td>{{ $dtuserppicplant3->plant }}</td>
                                <td>{{ $dtuserppicplant3->email }}</td>
                                

                                @if($dtuserppicplant3->role == 4)
                                <td>
                                    <span class="badge text-bg-warning"">PPIC PLANT 3</span>
                                </td>
                                @endif

                                <td>
                                    <form action="/postdeletedatauserppicplant3/adm/{{ $dtuserppicplant3->url_unique }}" method="post">
                                        @csrf 
                                        @method('DELETE')

                                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Apakah anda yakin ingin menghapus data user ini ?')">Hapus Akun User</button>
                                    </form>
                                </td>
                              </tr>
                              <?php $no++;?>
    ...
    <script>
        ...
        function previewImage() {
            const image = document.querySelector('#imageppicplant3');
